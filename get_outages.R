...
from <- unix_from_date(now() - days(1) + hours(8)) #VERIFICAR ESO EL DESPLAZAMIENTO, LA FUNCION UNIX_... PARECE TENER UN OFFSET
...
if(nrow(providers) > 0){
providers <- providers %>%  group_by(org) %>% 
  slice_max(value) %>% 
